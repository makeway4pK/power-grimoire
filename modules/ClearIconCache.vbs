...
Sub ExitExplorerShell()
	...
	rtnStatus = MsgBox (strmsg, vbYesNo, "Clearing the Icon Cache")
...
If iDirtyFlags = 1 Then
	rtnStatus = MsgBox ("Some programs are still using the IconCache.db in LOCALAPPDATA. Close all programs and try again", vbOKonly, "Clearing the Icon Cache")
End If

If iDirtyFlags2 = 1 Then
	If iDirtyFlags <> 1  Then
		rtnStatus = MsgBox ("Some programs are still using the cache in Location 2. Close all programs and try again", vbOKOnly, "Clearing the Icon Cache")
	End If
End If

If NOT(iDirtyFlags = 0 OR iDirtyFlags2 = 0) Then
 	MsgBox "Could not clear all the icon caches.", vbOKOnly, "Clearing the icon cache"
End If
